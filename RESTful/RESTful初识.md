...
```
GET    /classes：列出所有班级
POST   /classes：新建一个班级Copy to clipboardErrorCopied
```
...
DELETE classes/classId/teachers/ID：删除某个指定班级下的指定的老师的信息Copy to clipboardErrorCopied
```
...
/getAllclasses
/createNewclass
/deleteAllActiveclassesCopy to clipboardErrorCopied
```
...
```
GET    /classes?page=1&size=10 //指定第1页，每页10个数据Copy to clipboardErrorCopied
```
