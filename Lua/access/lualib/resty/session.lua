...
function session.new(opts)
    ...
    local ide, iden = prequire("lualib.resty.session.identifiers.", opts.identifier or defaults.identifier, "random")
    local ser, sern = prequire("lualib.resty.session.serializers.", opts.serializer or defaults.serializer, "json")
    local com, comn = prequire("lualib.resty.session.compressors.", opts.compressor or defaults.compressor, "none")
    local enc, encn = prequire("lualib.resty.session.encoders.",    opts.encoder    or defaults.encoder,    "base64")
    local cip, cipn = prequire("lualib.resty.session.ciphers.",     opts.cipher     or defaults.cipher,     "aes")
    local sto, ston = prequire("lualib.resty.session.storage.",     opts.storage    or defaults.storage,    "cookie")
    local str, strn = prequire("lualib.resty.session.strategies.",  opts.strategy   or defaults.strategy,   "default")
    local hma, hman = prequire("lualib.resty.session.hmac.",        opts.hmac       or defaults.hmac,       "sha1")
